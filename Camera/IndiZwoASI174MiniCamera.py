# FC, aout 2022, à partir des scripts de Thibault Notargiacomo
...
class IndiZwoASI174MiniCamera(IndiDevice):
    """ Indi Camera """
    ...
    def get_frame_depth(self):
        return self.get_switch('CCD_CAPTURE_FORMAT')
        # , ['ASI_IMG_RAW16']

    def set_frame_depth(self, depth):
        """
        Frame depth can be 8 or 16 bits/pixel
        """
        if (depth == 8):
            self.set_switch('CCD_CAPTURE_FORMAT', ['ASI_IMG_RAW8'], sync=True, timeout=self.defaultTimeout)
        if (depth == 16):
            self.set_switch('CCD_CAPTURE_FORMAT', ['ASI_IMG_RAW16'], sync=True, timeout=self.defaultTimeout)
